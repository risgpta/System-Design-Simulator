...
function MetricCard({
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-2 rounded-lg border ${borders[status]} transition-colors duration-500`}
...
export default function TopBar({ metrics }: { metrics: SimulationMetrics }) {
  const latencyStatus: Status =
    metrics.avgLatency > 500 ? "critical" : metrics.avgLatency > 150 ? "warning" : "normal";
  const errorStatus: Status =
    metrics.errorRate > 0.2 ? "critical" : metrics.errorRate > 0.05 ? "warning" : "normal";
  const successStatus: Status =
    metrics.successRate < 0.8 ? "critical" : metrics.successRate < 0.95 ? "warning" : "normal";

  return (
    <div
      className="flex items-center gap-3 px-5 h-[60px] border-b border-white/10 bg-black/50 backdrop-blur-sm flex-shrink-0"
      data-testid="top-bar"
    >
